...
export const createTaskSchema = z.object({
  space_id: z.string().uuid({ message: "Nieprawidłowy format ID przestrzeni" }),
  name: z
    .string()
    .min(1, { message: "Nazwa zadania jest wymagana" })
    .max(200, { message: "Nazwa zadania może mieć maksymalnie 200 znaków" }),
  recurrence_value: z.number().int().positive({ message: "Wartość cykliczności musi być liczbą dodatnią" }),
